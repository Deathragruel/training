...
class Tree(pygame.sprite.Sprite):
    def __init__(self, pos, group):
        ...
        self.image = pygame.transform.smoothscale(self.image, (91, 96))
...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos, group):
        ...
        self.image = pygame.transform.smoothscale(self.image, (45, 48))
...
class CameraGroup(pygame.sprite.Group):
    def __init__(self):
        ...
        # box setup
        self.camera_borders = {'left': 156, 'right': 156, 'top': 75, 'bottom':
                75,}
        ...
        self.ground_surface = pygame.transform.smoothscale(self.ground_surface, (2595, 2400))
        ...
        self.internal_surface = pygame.Surface(self.internal_surface_size,
                pygame.SRCALPHA)
        self.internal_rect = self.internal_surface.get_rect(center =
    ...
    def mouse_control(self):
        # mouse_pos can only be checked once per frame, hence multiple if
        # statements
        # moreover, mouse is only bound to the original game window so we
        # cannot use camera_rect
        ...
        if top_border < mouse.y <  bottom_border:
            ...
            if mouse.x < left_border:
                mouse_offset_vector = mouse - pygame.math.Vector2(left_border,
                        top_border)
                pygame.mouse.set_pos((left_border, top_border))
            if mouse.x > right_border:
                mouse_offset_vector = mouse - pygame.math.Vector2(right_border,
                        top_border)
...
for i in range(20):
    random_x = randint(781, 1562)
    random_y = randint(750, 1500)
...
while True:
    ...
    camera_group.update()
    # Overwriting the basic draw functionality
